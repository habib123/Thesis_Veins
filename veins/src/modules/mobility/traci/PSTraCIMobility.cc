...
Define_Module(Veins::PSTraCIMobility);

const simsignalwrap_t PSTraCIMobility::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
...
void PSTraCIMobility::Statistics::initialize()
{
...
void PSTraCIMobility::Statistics::watch(cSimpleModule& )
{
    WATCH(totalTime);
    WATCH(minSpeed);
    WATCH(maxSpeed);
...
void PSTraCIMobility::Statistics::recordScalars(cSimpleModule& module)
{
...
void PSTraCIMobility::initialize(int stage)
{
    if (stage == 0)
    {
...
void PSTraCIMobility::handleSelfMsg(cMessage *msg)
{
...
void PSTraCIMobility::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double angle)
{
...
void PSTraCIMobility::nextPosition(const Coord& position, std::string road_id, double speed, double angle, TraCIScenarioManager::VehicleSignal signals)
{
...
void PSTraCIMobility::changePosition()
{
...
void PSTraCIMobility::changeParkingState(bool newState)
{
    isParking = newState;
    emit(parkingStateChangedSignal, this);
}

void PSTraCIMobility::updateDisplayString() {
    ASSERT(-M_PI <= angle);
    ASSERT(angle < M_PI);
...
void PSTraCIMobility::fixIfHostGetsOutside()
{
...
double PSTraCIMobility::calculateCO2emission(double v, double a) const {
...
Coord PSTraCIMobility::calculateAntennaPosition(const Coord& vehiclePos) const {
    Coord corPos;
